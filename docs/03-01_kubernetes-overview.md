...
### 3.1.5 파드의 생명주기와 쿠버네티스 구성 요소

#### 쿠버네티스의 가장 큰 장점

> 각 구성 요소의 역할이 명확하게 분리되어 있고, 각자 맡은 일만 수행함으로써 클러스터 시스템을 안정적으로 운영할 수 있음.

이러한 구조는 **마이크로서비스 아키텍처(MSA)**의 철학과도 유사함 — 유기적으로 연결되되, 각 요소는 독립적으로 동작함.

------

#### 파드 생명주기와 구성 요소 흐름

참고: 『컨테이너 인프라 환경 구축을 위한 쿠버네티스/도커』, p.136 그림 3-15 파드의 생명주기

1. `kubectl`을 통해 **API 서버**에 파드 생성 요청
2. **API 서버**는 요청 내용을 **etcd**에 저장해 클러스터 상태를 최신으로 유지
    → 상태 업데이트는 항상 API 서버를 통해 etcd에 기록됨
3. **컨트롤러 매니저**는 생성 요청을 감지하고 파드를 생성, 그 상태를 다시 API 서버에 전달
    → 아직 워커 노드에 파드가 할당되진 않음
4. **스케줄러**가 새 파드를 감지하고, 조건을 바탕으로 알맞은 **워커 노드**를 선택
5. **스케줄러**는 선택된 워커 노드의 **kubelet**에 파드 실행 요청
6. **kubelet**은 요청 받은 내용을 **컨테이너 런타임(CRI)**에 전달
7. **컨테이너 런타임**이 파드를 생성
8. 파드가 "Ready" 상태가 되어 실제 서비스가 가능해짐

------

#### 쿠버네티스는 선언형 시스템

- 쿠버네티스는 순차 명령형 워크플로우가 아니라 **선언형 시스템(declarative system)**
- 사용자는 “원하는 상태(desired state)”를 API 서버에 **선언**
- 다른 구성 요소는 이를 확인하고 현재 상태(actual state)와 맞추려 노력
- `etcd`는 API 서버의 현재 상태값을 보존하는 역할을 담당
- **마스터 노드**의 구성은 선언적 시스템이 적합하며,
   **워커 노드**는 kubelet과 컨테이너 런타임을 통해 명령형 동작을 따름
...
- 마스터 노드 외부에도 설치 가능
- API 서버의 접속 정보만 있으면 클러스터 외부에서도 제어 가능
- 사용자와 쿠버네티스 클러스터 사이의 인터페이스 역할

#### 🔹 kube-proxy

- 워커 노드에 배치
- 파드 간 네트워크 통신을 가능하게 함
- 실제 통신은 `br_netfilter` 및 `iptables`로 처리



### 3.1.6 쿠버네티스 구성 요소의 기능 검증

#### kubectl

- **쿠버네티스 클러스터에 명령을 내리는 CLI 도구**
- 반드시 마스터 노드에 설치되어야 하는 것은 아님
- **API 서버의 접속 정보만 있으면, 외부에서도 사용 가능**
- 주요 역할: 파드 생성, 상태 확인, 리소스 수정 등
- 실제 명령은 API 서버를 통해 클러스터 내부에 전달됨

#### kube-proxy

- **파드 간 네트워크 통신 담당**
- 각 워커 노드에 배치되어 **가상 IP와 포트 포워딩** 설정
- 내부적으로 `iptables` 또는 `ipvs` 등을 사용하여
   파드와 서비스 간 통신 라우팅을 처리
- 클러스터 내부에서 **서비스 디스커버리와 부하 분산** 지원